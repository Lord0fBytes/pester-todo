...
- `MARKETING_VERSION` is the release version, such as `0.6.1`.
- `CURRENT_PROJECT_VERSION` is the incrementing build number.

The app displays both together, such as `0.6.1-0044`. The displayed value comes from the built app bundle, so it identifies what is actually installed on the phone.
