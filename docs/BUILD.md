...
- `MARKETING_VERSION` is the release version, such as `0.5.0`.
- `CURRENT_PROJECT_VERSION` is the incrementing build number.

The app displays both together, such as `0.5.0-0018`. The displayed value comes from the built app bundle, so it identifies what is actually installed on the phone.
