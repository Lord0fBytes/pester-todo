...
- `MARKETING_VERSION` is the release version, such as `0.2.0`.
- `CURRENT_PROJECT_VERSION` is the incrementing build number.

The app displays both together as `0.2.0-0003`. The displayed value comes from the built app bundle, so it identifies what is actually installed on the phone.
